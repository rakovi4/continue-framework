...
**A repo adopting this rule backfills once.** The archive fires only on a story's completion commit, so every story already in the **Done** table when the rule lands predates it and keeps its folder under `stories/` — which makes `ls stories/` misreport shipped work as in flight, permanently, in exactly the repos that have the most of it. The one-time fix is a sweep, not a mechanism: for each **Done** row whose folder is still under `stories/`, `git mv` it into `stories/done/` (re-pointing its mockups' `<script src>` prefixes for the added depth — `.claude/templates/ui/mockup-generation-rules.md`, "Script import path"), in a single commit. Do it when adopting the rule; after that the completion commit keeps the invariant true on its own.
