...
- **module**: backend module/adapter name | frontend
- **focus**: boolean (restrict to touched production files)
- **scope**: explicit owned production paths and baseline revision for staged lanes
- **report_only**: required for staged lanes; return findings without tracking edits
...
In staged backend or frontend lane report-only mode, return every gap to the
coordinator. Do not edit `progress.md`; the coordinator applies admission and owns
follow-up insertion before stage completion.

1. **Run coverage for primary module** — read `technology.md` and resolve the backend or frontend concern. Use its coverage tooling and report conventions (backend commands: `.claude/tech/{backend}/templates/testing/coverage-commands.md`); load `.claude/templates/testing/coverage-commands.md` for scope and module mapping. Missing tooling is a failed prerequisite, never a clean report.
...
4. **Check results** — report clean only when every scoped executable file appears in the reports with full line and branch coverage; assess branchless files by line coverage
...
8. **Return or record gaps** — in report-only mode return findings; otherwise insert reachable-gap steps using the format below
