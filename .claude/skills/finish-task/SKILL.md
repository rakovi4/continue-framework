...
1. Resolve `task N` with `/continue`'s task lookup rules. If already archived, report
   its location; archive an active task with no open checkbox.
...
4. Execute the next work with the template's task-type route. Keep every applicable
   atomic unit, approval, test, work-log, and commit boundary; replace only
...
- QA cases are serial; do not fan them out.
