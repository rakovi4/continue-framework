...
| `stage-1 acceptance RED + contract design` | **Inline coordinator.** Load `parallel-backend-stages.md`; declare disjoint ownership, dispatch `red-agent` and a general worker executing `/design-preview` concurrent mode, require the returned Stage 2 lane plan to pass its architectural-unit, writable-delta versus frozen-surface, overlap, and no-dependency gates, gather both committed results, record the durable plan, advance to the approval checkbox in a coordinator commit, then stop for user review |
