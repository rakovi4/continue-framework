...
| 25 | Se | Missing test description annotation | (read each test method: check for display annotation containing spec text) | test class only | A single-case method carries the scenario text. In a multi-case acceptance target, the group carries the scenario title and every method carries a complete Given/When/Then slice. |
...
| 29 | P  | Stacked acceptance test | (read whether one acceptance method copies a large multi-phase scenario, contains multiple execution phases, or mixes unrelated outcomes) | acceptance test class only | Split it into small, focused methods inside the same scenario target; do not create progress blocks per method. |
| 30 | P  | Fragmented or coupled acceptance case | (read every method in a multi-case acceptance target and its setup) | acceptance test class only | Each case must carry complete context, run independently, and map the scenario's Then clauses collectively without relying on order or sibling state. |
