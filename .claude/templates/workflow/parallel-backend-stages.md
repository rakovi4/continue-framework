...
Stage 1 closes with a user review of the acceptance test and the frozen interfaces.
Approval freezes the use-case and port contracts for Stage 2. The coordinator
...
target the same test file into one lane, or queue those lanes serially; concurrent
writers never share a path. A follow-up owns the named test file, commits under the
same lock, and must pass before Stage 2 advances.
