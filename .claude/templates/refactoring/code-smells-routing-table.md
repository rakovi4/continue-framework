...
All templates live in `.claude/templates/refactoring/`. Select by responsibility
using `scan-checklist.md`, on client and server alike. Examples in older templates
illustrate a pattern; use the active binding for syntax and equivalent constructs.
Storage, transport, and presentation mechanisms apply only where present.
Every retained candidate needs the same `restraint.md` evidence.
...
## Shared Code Smells and Boundary Patterns
...
| Computation interleaved with side effects | Extract model decisions and transitions from orchestration; preserve response dependencies, freshness checks, and observable order | `extract-method.md` |
...
| Sequential guards for one concern (find + validate) | Extract a named decision or preserve an existing throwing guard contract; returning from a helper must not let a rejected caller continue | `extract-method.md` |
...
| Test helper duplicates production computation | Keep expected values independent; use literal oracles. Reuse setup behavior only when it cannot determine the expected result | (inline — classify setup versus oracle first) |
...
| Suspected unreachable branch | Prove unreachability against contracts before removing; a coverage gap alone cannot establish dead code | (inline — verify, then simplify) |
| Long function, method, callback, or effect body (>10 lines) | Extract named responsibilities; a retained body requires concrete alternatives and KEEP evidence | `extract-method.md` |
...
| Sequential blocks with distinct responsibilities | Extract named steps even when values pass between them; shared state is not a cohesion exemption | `extract-method.md` |
| Disposable local alias | Inline if it names no meaningful value, snapshot, identity, or effect result | `simplify-expressions.md` |
| Effect hidden inside a pure expression | Isolate actual effects or time-dependent observations; inspect behavior rather than injection | `simplify-expressions.md` |
| Distinct or repeated computation mixed into a caller | Extract to its owner; retain simple meaningful local names | `extract-method.md` |
...
| Redundant functional wrapper | Remove unnecessary forwarding; retain meaningful captured identity, cleanup, or lifetime under evidenced restraint | `simplify-expressions.md` |
...
| Related implementations duplicating behavior | Extract a focused shared owner using composition or inheritance appropriate to the binding | `extract-class.md` |
...
## Additional Presentation Patterns
...
| Render branches mixing distinct view responsibilities | Extract cohesive views; preserve clear guard returns and avoid mutable render accumulators | `extract-component.md` |
| Domain or repeated computations inside rendering | Move behavior to its model or named presentation helper; retain simple local names and framework bindings | `extract-component.md` |
| Component reused across features | Move to Shared UI | `extract-shared-ui.md` |
| Business logic in presentation | Move to the pure capability model; lifecycle coordination belongs in the controller | (inline — apply frontend role map) |
| Transport or multi-step effects in presentation | Move transport to an adapter and operation sequencing to its controller | (inline — apply frontend role map) |
...
| Long test body mixing setup, action, and observation detail | Extract fixtures and assertion helpers with independent expected values; apply in every test binding | `extract-test-fixture.md` |
