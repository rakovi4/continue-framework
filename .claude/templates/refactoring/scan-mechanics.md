...
| A59 | Production comments and comment bloat | Count production source lines, comment-only lines, and each contiguous comment block (`start–end`, line count). Quote or tightly identify every block and classify it as *code narration*, *section label*, *implementation history/test anecdote*, *rationale/constraint*, *contract*, *tooling directive*, or *safety warning*. For every proposed `KEEP`, state the present maintainer decision it changes and why names, types, tests, version control, or owning documentation cannot preserve it. | Any narration, section label, implementation history, test anecdote, multi-paragraph comment, rationale longer than two source lines, or `KEEP` without both necessity answers. Replace narration with code; compress an irreducible rationale to at most two lines; relocate a durable external contract to its owning documentation. Tooling directives and safety warnings may exceed two lines only when truncation would break the tool or hide the hazard. |
