...
**Placement, and why nothing waits.** `/continue` appends the block at the end of the matching `## Tier N — {Category} Scenarios ({file})` section for the newcomer's own tier, in the same `review-fix:` commit that writes the test file. If that section does not exist, create it in `bootstrapping.md`'s category order (`01_API`, `06_Integration`, `02_UI`, `05_Security`, `03_Load`, `04_Infrastructure`), a new Tier 2 section after `## Harvest — Tier 1 → Tier 2`. Two shapes put the block **above** the current `[~]`: a `BLOCK`-promoted **Tier 1** scenario found during Tier 2 work (the whole Tier 1 region is above the cursor), and a Tier 2 newcomer whose category sorts earlier than the cursor's. Neither defers, because the passes fire only at a **boundary** — the block they read has no open step left, and the only `[~]` in the file is the first, untouched step of the *next* block. There is no in-flight cycle to strand, so the plan write and the test file always land together and the `stories.md` recount happens with them.

**The cursor still moves onto the newcomer.** Check 3 of the plan-integrity check is purely syntactic — no `[ ]` above the file's first `[~]` — so a `[ ]` block dropped into an earlier section fails it even at a boundary, where the *harm* is gone but the check cannot tell. So insert the block, mark its first step `[~]`, and return the previously-`[~]` step to `[ ]`. That satisfies the check as written, needs no exemption, and is the right sequencing anyway: a promoted Tier 1 scenario genuinely is the next work unit, and everything above it is already `[x]`/`[S]`. On a work item's **last** boundary there is no next block to take the cursor from — there the `review-fix:` commit reopens the item and the newcomer's first step becomes the plan's only `[~]` (`.claude/guidelines/review-passes-detail.md`, "A finding can land after its item was flipped to Done").

Appending *below* the cursor instead is the alternative, and it fails in kind: it either files a scenario under a category section it does not belong to (falsifying the `stories.md` per-category cells) or puts a Tier 1 section under the harvest boundary, falsifying tier-major order for every later resume, which then fails check 2 forever.
