...
- [ ] Generalize `/continue` SKILL.md step 11 from "move the task folder" to the work item's folder — a story moves `ProductSpecification/stories/NN-story-name/` → `ProductSpecification/stories/done/NN-story-name/` in the same behavior commit that moves its `stories.md` row to the **Done** table, under the same staged-`progress.md` gate (`git show :`/`git diff --cached`, zero `[ ]`/`[~]` remaining). Update Pre-Commit Checklist item 7 to cover both item types, extend the Triage reopen clause so a mid-cycle Tier 2 finding moves the folder back out of `done/` in the `review-fix:` commit, and update `.claude/rules/workflow.md` + `.claude/templates/workflow/stories-md-format.md` ("Story completion") to state the folder move alongside the row move
